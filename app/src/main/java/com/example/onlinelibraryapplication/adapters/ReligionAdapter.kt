...
lateinit var contextReligionAdapter: Context

class ReligionAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<ReligionAdapter.MyViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextReligionAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.description1.text = currentItem.description.substring(0, 60).plus("...")
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextReligionAdapter, BookActivity::class.java)
            ...
            contextReligionAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextReligionAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
    }
