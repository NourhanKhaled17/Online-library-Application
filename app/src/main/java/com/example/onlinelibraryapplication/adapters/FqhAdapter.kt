...
lateinit var contextFqhAdapter: Context

class FqhAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<FqhAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextFqhAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextFqhAdapter, BookActivity::class.java)
            ...
            contextFqhAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextFqhAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
