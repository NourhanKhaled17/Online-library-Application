...
lateinit var contextCrimeAdapter: Context

class CrimeAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<CrimeAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextCrimeAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextCrimeAdapter, BookActivity::class.java)
            ...
            contextCrimeAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextCrimeAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
