...
import com.example.onlinelibraryapplication.views.HistoryActivity
import com.google.android.material.imageview.ShapeableImageView

@SuppressLint("StaticFieldLeak")
lateinit var contextHistoryAdapter: Context

class HistoryAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<HistoryAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextHistoryAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextHistoryAdapter, BookActivity::class.java)
            ...
            contextHistoryAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextHistoryAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
    }
