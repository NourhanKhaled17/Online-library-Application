...
lateinit var contextFearAdapter: Context

class FearAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<FearAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextFearAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextFearAdapter, BookActivity::class.java)
            ...
            contextFearAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextFearAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
