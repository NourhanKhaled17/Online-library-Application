...
lateinit var contextFantasyAdapter: Context

class FantasyAdapter(private val fieldsList: ArrayList<FieldModel>) :
    RecyclerView.Adapter<FantasyAdapter.MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.category_list_item, parent, false)
        contextFantasyAdapter = parent.context
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.bookCard.setOnClickListener {
            ...
            val intent = Intent(contextFantasyAdapter, BookActivity::class.java)
            ...
            contextFantasyAdapter.startActivity(intent)
        }

        holder.goCard.setOnClickListener {
            addItem(currentItem)
            Toast.makeText(
                contextFantasyAdapter, " تم إضافة ${holder.heading1.text} إلي العربة", Toast.LENGTH_SHORT
            ).show()
        }
    }
